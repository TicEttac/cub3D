...
void	pass_image(t_tex *tex, t_struct *view, int vsl)
{
	int			x = 0;
	int			y = 0;
	int			bpp = 32;
	...
	{
		//printf("j=%d sl=%d\n", y, sl);
		view->tab[x + (y * tex->width)] = ((unsigned int*)tex->tab)[x + (y * tex->width)];
		//printf("val=%u\n", ((unsigned int*)tex->tab)[x + (y * tex->width)]);
		x++;
		if (x == tex->width)
		{
			printf("reset\n");
...
int		main(int ac, char **av)
{
	...
	view.tab = (unsigned int *)mlx_get_data_addr(view.img, &bpp, &sl, &en);
	pass_image(&tex, &view, sl);
	mlx_put_image_to_window(view.mlx, view.win, view.tab, 0, 0);
